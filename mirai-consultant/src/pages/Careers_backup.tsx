  ...
  return (
    <div className="min-h-screen pt-20 relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-violet-500/5 rounded-full blur-3xl animate-pulse delay-500" />
      </div>
      {/* Hero Banner */}
      <motion.div
        initial={{ opacity: 0 }}
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8 mb-16">
          {benefits.map((benefit, index) => (
            <motion.div
              ...
              className="group relative bg-gradient-to-br from-slate-900/90 to-slate-800/70 backdrop-blur-xl border border-white/10 rounded-3xl p-8 hover:border-fuchsia-500/50 transition-all duration-500 cursor-pointer overflow-hidden"
            >
              {/* Animated Background */}
        ...
        <div className="space-y-6">
          {filteredRoles.map((role, index) => (
            <motion.div
              ...
              viewport={{ once: true }}
              className="group relative bg-gradient-to-br from-slate-900/95 to-slate-800/95 backdrop-blur-xl border border-white/10 rounded-3xl p-8 lg:p-12 hover:border-blue-500/60 hover:shadow-[0_0_60px_rgba(37,99,235,0.2)] transition-all duration-500 overflow-hidden"
            >
              {/* Animated Background */}
              <div className="absolute inset-0 bg-gradient-to-r from-blue-500/0 via-indigo-500/5 to-sky-500/0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 rounded-3xl" />
              <motion.div 
                className="absolute -inset-1 bg-gradient-to-r from-blue-500/10 via-indigo-500/10 to-sky-500/10 rounded-3xl blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500"
              ...
              <div className="relative">
                ...
                <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-6 mb-8">
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 mb-4">
                      <span className="px-3 py-1.5 rounded-lg bg-gradient-to-r from-blue-500/20 to-indigo-500/20 border border-blue-500/30 text-blue-300 text-xs font-bold uppercase tracking-wider">
                        {role.category}
                      </span>
                      <span className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-slate-300 text-xs font-semibold">
                        {role.department}
                      </span>
                    </div>

                    <h3 className="text-3xl lg:text-4xl font-black text-white mb-5 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-blue-400 group-hover:to-sky-400 transition-all duration-300">
                      {role.title}
                    </h3>
                    ...
                    <div className="flex flex-wrap items-center gap-3 mb-5">
                      <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-colors">
                        ...
                        <span className="text-sm font-semibold text-white">{role.location}</span>
                      </div>
                      <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-colors">
                        <Clock className="h-4 w-4 text-sky-400" />
                        <span className="text-sm font-semibold text-white">{role.type}</span>
                      </div>
                      <div className="flex items-center gap-2 px-4 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 transition-colors">
                        <TrendingUp className="h-4 w-4 text-violet-400" />
                        <span className="text-sm font-semibold text-white">{role.experience}</span>
                      </div>
                    </div>
                    
                    <p className="text-slate-300 text-lg leading-relaxed mb-6">{role.summary}</p>
                  </div>
                  ...
                  <div className="flex flex-col gap-3 lg:min-w-[280px]">
                    ...
                    <motion.button
                      ...
                      whileTap={{ scale: 0.98 }}
                      className="flex items-center justify-center gap-2.5 px-6 py-3.5 rounded-xl bg-slate-700 border-2 border-slate-600 hover:bg-slate-600 hover:border-slate-500 text-white font-bold transition-all duration-300 shadow-lg shadow-slate-900/50"
                    >
                      {copiedEmail === role.id ? (
                        <>
                ...
                {/* Details Grid */}
                <div className="grid lg:grid-cols-2 gap-8 pt-8 border-t border-white/10">
                  <div className="space-y-5">
                    <div className="flex items-center gap-3 mb-4">
                      <div className="h-10 w-1.5 bg-gradient-to-b from-blue-500 via-indigo-500 to-blue-500 rounded-full shadow-lg shadow-blue-500/50" />
                      <h4 className="text-xl font-bold text-white">Key Responsibilities</h4>
                    </div>
                    <ul className="space-y-4">
                      {role.responsibilities.map((item, i) => (
                        ...
                        >
                          <div className="flex-shrink-0 w-6 h-6 rounded-lg bg-gradient-to-br from-blue-500/20 to-indigo-500/20 flex items-center justify-center border border-blue-500/30 group-hover/item:scale-110 transition-transform">
                  ...
                  <div className="space-y-5">
                    <div className="flex items-center gap-3 mb-4">
                      <div className="h-10 w-1.5 bg-gradient-to-b from-sky-500 via-violet-500 to-sky-500 rounded-full shadow-lg shadow-sky-500/50" />
                      <h4 className="text-xl font-bold text-white">Requirements</h4>
                    </div>
                    <ul className="space-y-4">
                      {role.requirements.map((item, i) => (
                        <motion.li 
                          ...
                          className="flex items-start gap-4 text-slate-300 group/item hover:text-white transition-colors"
                        >
                          <div className="flex-shrink-0 w-6 h-6 rounded-lg bg-gradient-to-br from-sky-500/20 to-violet-500/20 flex items-center justify-center border border-sky-500/30 group-hover/item:scale-110 transition-transform">
                            <div className="h-2 w-2 rounded-full bg-sky-400 shadow-lg shadow-sky-500/50" />
                          </div>
                          <span className="text-sm leading-relaxed">{item}</span>
                        </motion.li>
                      ))}
                ...
                <motion.div 
                  ...
                  viewport={{ once: true }}
                  className="mt-8 pt-8 border-t border-white/10 flex items-center justify-between gap-4 flex-wrap"
                >
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-500/20 to-sky-500/20 flex items-center justify-center border border-white/10">
                      <Briefcase className="h-6 w-6 text-blue-400" />
                    </div>
                    <div>
                      <p className="text-sm font-semibold text-white">Ready to apply?</p>
                      <p className="text-xs text-slate-400">Send your resume to: <span className="text-fuchsia-400 font-mono">{role.email}</span></p>
                    </div>
                  </div>
                  <div className="text-sm text-slate-500">
                    Position ID: <span className="font-mono text-slate-400">{role.id}</span>
                  </div>
                </motion.div>
              </div>
            </motion.div>
